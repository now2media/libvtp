...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), m_lFile(new LFile()),
      m_lPreview(new LPreview()), m_lOutput(new LOutput()),
      m_uiTimer(new QTimer(this)), m_statsTimer(new QTimer(this)) {
  ui->setupUi(this);
  setWindowTitle("Broadcast Playout Output Control Dashboard");

  setStyleSheet(
      "QMainWindow { background-color: #0f172a; color: #f8fafc; }"
      "QGroupBox { border: 1px solid rgba(255,255,255,0.1); border-radius: "
      "12px; margin-top: 15px; font-weight: bold; padding: 15px; "
      ...
      "color: #f8fafc; padding: 8px; border-radius: 6px; min-width: 150px; }"
      "QComboBox:disabled { background-color: #1e293b; color: #64748b; "
      "border-color: #334155; }"
      "QPushButton { background-color: #334155; color: #f8fafc; border: none; "
      "padding: 10px 20px; border-radius: 6px; font-weight: bold; font-size: "
      "13px; }"
  ...
  connect(ui->enableOutputCheck, &QCheckBox::toggled, this,
          &MainWindow::onOutputToggled);
...
void MainWindow::onOutputToggled(bool checked) {
  if (checked) {
    ...
    ui->liveStatusLbl->setStyleSheet(
        "color: #ef4444; font-weight: bold; font-size: 15px;");
  } else {
    m_lOutput->setEnabled(false);
...
void MainWindow::onSelectFileClicked() {
  QString filePath =
      QFileDialog::getOpenFileName(this, "Select Playout Media File", "",
                                   "Media Files (*.mp4 *.mkv *.mov *.avi *.ts "
                                   "*.mxf *.mp3 *.wav *.aac);;All Files (*)");
  if (!filePath.isEmpty()) {
    m_lFile->stop();
...
void MainWindow::updateUI() {
  ...
  if (durMs > 0) {
    ...
    ui->timecodeLbl->setText(msToTimecode(curMs, fps) + " / " +
...
void MainWindow::updateOutputStats() {
  ...
  ui->statsResolutionVal->setText(
      QString("%1 x %2").arg(stats.width).arg(stats.height));
  ui->statsFpsVal->setText(QString("%1 FPS").arg(stats.fps, 0, 'f', 2));
  ui->statsAudioVal->setText(QString("%1 Hz, %2 CH")
                                 .arg(stats.audioSampleRate)
                                 .arg(stats.audioChannels));
...
QString MainWindow::msToTimecode(double ms, double fps) {
  ...
  ss_tc << std::setfill('0') << std::setw(2) << hh << ":" << std::setfill('0')
