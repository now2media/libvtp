...
VTPDiscoveryAdvertiser::VTPDiscoveryAdvertiser(const std::string& name, uint16_t port, int width, int height, int fps, bool enable_alpha, bool enable_audio)
    : name_(name), port_(port), width_(width), height_(height), fps_(fps), enable_alpha_(enable_alpha), enable_audio_(enable_audio) {}

VTPDiscoveryAdvertiser::~VTPDiscoveryAdvertiser() {
    Stop();
}
...
void VTPDiscoveryAdvertiser::Run() {
    try {
        ...
        // IP will be inferred by receiver from UDP sender endpoint

        while (running_) {
            asio::error_code ec;
            socket.send_to(asio::buffer(&adv_info, sizeof(adv_info)), multicast_endpoint, 0, ec);
            if (ec) {
                std::cerr << "[VTP Discovery] Send failed: " << ec.message() << std::endl;
            }

            // Sleep for 1.5 seconds or until stopped
            for (int i = 0; i < 15 && running_; ++i) {
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
        }
    } catch (const std::exception& e) {
        std::cerr << "[VTP Discovery] Advertiser exception: " << e.what() << std::endl;
    }
}
    ...
    sources_.erase(
        std::remove_if(sources_.begin(), sources_.end(),
            [now](const DiscoveredSource& src) {
                // If we haven't seen the advertiser in 3.5 seconds, remove it
                return std::chrono::duration_cast<std::chrono::seconds>(now - src.last_seen).count() >= 3.5;
            }),
        sources_.end());
}
...
void VTPDiscoveryListener::Run() {
    try {
        ...
        socket.set_option(asio::ip::multicast::join_group(asio::ip::make_address(MULTICAST_ADDRESS)));

        // Set non-blocking/timeout options for receive loop
        ...
        while (running_) {
            ...
            size_t bytes_recvd = socket.receive_from(
                asio::buffer(&recv_buffer, sizeof(recv_buffer)), sender_endpoint, 0, ec);

            if (!ec && bytes_recvd == sizeof(VTPSource)) {
                std::lock_guard<std::mutex> lock(mutex_);
                
                // Set the correct IP from sender endpoint
                std::string sender_ip = sender_endpoint.address().to_string();
                std::strncpy(recv_buffer.ip, sender_ip.c_str(), sizeof(recv_buffer.ip) - 1);
        ...
        std::cerr << "[VTP Discovery] Listener exception: " << e.what() << std::endl;
    }
}
