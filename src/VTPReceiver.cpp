...
struct VTPReceiver::Impl {
  ...
  std::vector<unsigned char>
      decompressed_frame_buffer_; // Stable buffer for client

  std::mutex audio_mutex_;
  std::condition_variable audio_cv_;
  std::queue<AudioFrameCache> audio_queue_;
  std::vector<unsigned char>
      decompressed_audio_buffer_; // Stable buffer for client
...
bool VTPReceiver::Connect(const std::string &ip, uint16_t port) {
  ...
  try {
    ...
    if (ec) {
      std::cerr << "[VTPReceiver] Connection to " << target_ip << ":"
                << target_port << " failed: " << ec.message() << std::endl;
      return false;
    }

    impl_->socket_.set_option(asio::ip::tcp::no_delay(true));
    impl_->connected_ = true;
    std::cout << "[VTPReceiver] Connected to VTP Sender at " << target_ip << ":"
              << target_port << std::endl;
    impl_->receive_thread_ =
        std::thread(&VTPReceiver::Impl::ReceiveLoop, impl_.get(), this);
    return true;
...
void VTPReceiver::Impl::ReceiveLoop(VTPReceiver *parent) {
  ...
  while (connected_) {
    try {
      ...
      if (ec) {
        if (connected_) {
          std::cerr << "[VTPReceiver] Error reading packet header: "
                    << ec.message() << std::endl;
        }
        break;
      }
      ...
      if (pkt_header.type == VTP_PACKET_VIDEO) {
        ...
        asio::read(
            socket_,
            asio::buffer(local_fill_jpeg_buf.data(), frame_hdr.fill_size), ec);
        if (ec)
          break;
        ...
        if (frame_hdr.has_alpha) {
          ...
          asio::read(
              socket_,
              asio::buffer(local_key_jpeg_buf.data(), frame_hdr.key_size), ec);
          if (ec)
            break;
        }
        ...
        int decompress_res =
            tjDecompress2(tj_decompressor_, local_fill_jpeg_buf.data(),
                          frame_hdr.fill_size, local_decompressed_buf.data(),
                          frame_hdr.width, 0, frame_hdr.height, tj_pf, 0);

        if (decompress_res == 0) {
          if (frame_hdr.has_alpha &&
              (frame_hdr.pixel_format == VTP_FORMAT_RGBA ||
               frame_hdr.pixel_format == VTP_FORMAT_BGRA)) {
            size_t num_pixels = frame_hdr.width * frame_hdr.height;
            local_alpha_buf.resize(num_pixels);

            int key_decompress_res = tjDecompress2(
                tj_decompressor_, local_key_jpeg_buf.data(), frame_hdr.key_size,
                local_alpha_buf.data(), frame_hdr.width, 0, frame_hdr.height,
                TJPF_GRAY, 0);
...

          {
            ...
            latest_video_frame_.format =
                static_cast<VTPPixelFormat>(frame_hdr.pixel_format);
          ...
          std::cerr << "[VTPReceiver] Decompression failed: "
                    << tjGetErrorStr2(tj_decompressor_) << std::endl;
        }
        ...
        asio::read(socket_,
                   asio::buffer(local_pcm_buf.data(), audio_meta.data_size),
                   ec);
        if (ec)
          break;
...
bool VTPReceiver::ReceiveFrame(VTPFrame *out_frame) {
  ...
  out_frame->format =
      static_cast<vtp_pixel_format_t>(impl_->latest_video_frame_.format);
...
bool VTPReceiver::IsConnected() const { return impl_->connected_; }
