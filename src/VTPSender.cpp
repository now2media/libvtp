...
struct VTPSender::Impl {
    ...
    void BroadcastBuffers(const std::vector<asio::const_buffer>& buffers) {
        ...
        for (auto it = clients_.begin(); it != clients_.end(); ) {
            ...
            if (ec) {
                std::cout << "[VTPSender] Client disconnected (write failed: " << ec.message() << ")" << std::endl;
...
VTPSender::VTPSender(const std::string& stream_name, int width, int height, int fps, bool enable_alpha, bool enable_audio)
...
bool VTPSender::Start() {
    ...
    try {
        // Find a free TCP port dynamically starting from 5000
        asio::ip::tcp::endpoint endpoint(asio::ip::tcp::v4(), 0);
        impl_->acceptor_ = std::make_unique<asio::ip::tcp::acceptor>(impl_->io_context_, endpoint);
        impl_->port_ = impl_->acceptor_->local_endpoint().port();

        std::cout << "[VTPSender] TCP listening on port " << impl_->port_ << std::endl;

        impl_->accept_thread_ = std::thread(&VTPSender::Impl::AcceptLoop, impl_.get());

        // Start Discovery Advertiser
        impl_->advertiser_ = std::make_unique<VTPDiscoveryAdvertiser>(
            impl_->stream_name_, impl_->port_, impl_->width_, impl_->height_, impl_->fps_, impl_->enable_alpha_, impl_->enable_audio_);
...
bool VTPSender::SendFrame(const unsigned char* data, VTPPixelFormat format, uint64_t timestamp_ns) {
    if (!impl_->running_ || !impl_->tj_compressor_) return false;

    // Check if there are any clients connected before doing the compression work
    {
        std::lock_guard<std::mutex> lock(impl_->clients_mutex_);
        if (impl_->clients_.empty()) return false;
    }
    ...
    int tj_pf = TJPF_RGB;
    switch (format) {
        case VTP_FORMAT_RGB:  tj_pf = TJPF_RGB;  break;
        case VTP_FORMAT_RGBA: tj_pf = TJPF_RGBA; break;
        case VTP_FORMAT_BGR:  tj_pf = TJPF_BGR;  break;
        case VTP_FORMAT_BGRA: tj_pf = TJPF_BGRA; break;
    }

    unsigned char* fill_jpeg_buf = nullptr;
    unsigned long fill_jpeg_size = 0;

    // Compress Fill (RGB/RGBA) frame
    int compress_res = tjCompress2(
        impl_->tj_compressor_,
        data,
        impl_->width_,
        0, // pitch (0 means auto-detect)
        impl_->height_,
        tj_pf,
        &fill_jpeg_buf,
        &fill_jpeg_size,
        TJSAMP_422, // subsampling (4:2:2 is broadcast quality, great balance of chroma resolution and bandwidth)
        impl_->scale_quality_.load(),  // Scale quality (dynamic)
        0 // flags
    );

    if (compress_res != 0) {
        std::cerr << "[VTPSender] Fill JPEG compression failed: " << tjGetErrorStr2(impl_->tj_compressor_) << std::endl;
        return false;
    }
    ...
    // Extract and compress Alpha (Key) if requested and supported by format
    bool actually_has_alpha = impl_->enable_alpha_ && (format == VTP_FORMAT_RGBA || format == VTP_FORMAT_BGRA);
    if (actually_has_alpha) {
        ...
        // Extract Alpha channel (4th byte in RGBA/BGRA)
        ...
        int key_compress_res = tjCompress2(
            impl_->tj_compressor_,
            impl_->alpha_extract_buffer_.data(),
            impl_->width_,
            0,
            impl_->height_,
            TJPF_GRAY,
        ...
        if (key_compress_res != 0) {
            std::cerr << "[VTPSender] Key JPEG compression failed: " << tjGetErrorStr2(impl_->tj_compressor_) << std::endl;
    ...
    // Packetize: Packet header prepended
    ...
    pkt_header.payload_size = sizeof(header) + fill_jpeg_size + (actually_has_alpha ? key_jpeg_size : 0);
    ...
    // Free TurboJPEG-allocated buffers
    if (fill_jpeg_buf) tjFree(fill_jpeg_buf);
    if (key_jpeg_buf) tjFree(key_jpeg_buf);

    return true;
}

bool VTPSender::SendAudio(const unsigned char* pcm_data, int sample_count, uint32_t sample_rate, uint8_t channels, uint64_t timestamp_ns) {
    if (!impl_->running_ || !impl_->enable_audio_) return false;

    // Check if there are any clients connected before doing any work
    {
        std::lock_guard<std::mutex> lock(impl_->clients_mutex_);
        if (impl_->clients_.empty()) return false;
    }
    ...
    // 16-bit Signed PCM is 2 bytes per sample per channel
    ...
    audio_meta.sample_format = 0; // 16-bit Signed PCM
...
uint16_t VTPSender::GetPort() const {
    return impl_->port_;
}

std::string VTPSender::GetStreamName() const {
    return impl_->stream_name_;
}
